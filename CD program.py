...
def my_program():
    try:
        ...
        def profile(given_account_number):
            ...
            if given_account_number not in registered_accounts:
                print("Invalid input!")
                given_account_number = int(input("Enter Number: - "))
                # Recursively call the profile function until a valid account number is provided
                profile(given_account_number)
                return
            else:
                # Switch to the second window (assuming it's the Edge browser)
            ...
            if given_account_number != 1:
                ...
                # List of account image paths
                account_name_image_path = Manage_account("ANIP")

                # Extract the account name from the path
                pro_name = account_name_image_path[given_account_number - 1].split("\\")
                pro_name = pro_name[2].split(".")
                # Attempt to detect the account name image, displaying a message if it's not found
                print(Fore.BLUE + Style.BRIGHT + f"Name Image path {account_name_image_path[given_account_number - 1]}" + Style.RESET_ALL)
                account_image = det_img(account_name_image_path[given_account_number - 1],
                            code_string=f"print('Please show {pro_name[0]}'s name! Searching to click on {pro_name[0]}'s profile')",
                            img_name=f"{pro_name[0]}")
                print(f"Finding {pro_name[0]}'s profile...")
                timewait = 0

                # Continue searching for the account name image until it's found
                scrol = -300
                while account_image is None:
                    pyautogui.sleep(sleep_time/2)
                    account_image = det_img(account_name_image_path[given_account_number - 1], code_string="pass",
                                img_name=f"{pro_name[0]}'s profile")

                    # If the account name image is still not found, attempt to open Edge
                    if account_image is None:
                        print("Please wait! Opening account's name...")
                        prof = Bunch_img_det("E:\\Det_img\\TXT Files\\Profile_icon.txt",
                                             code_string="pass", img_name="Profile Icon")
                        if prof is not None:
                            # Retry detecting the account name image after opening Edge
                            account_image = det_img(account_name_image_path[given_account_number - 1], code_string="pass",
                                        img_name=f"{pro_name[0]}'s profile")
                            if account_image is None:
                                pro_set_button = Bunch_img_det("E:\\Det_img\\TXT Files\\pro_set_button.txt", kt=2)
                                if pro_set_button is not None:
                        ...
                        if prof is None:
                            print("Opening Edge...")
                            # Wait for a few iterations before maximizing the window and opening a new tab
                            if timewait % 4 == 0:
                                pyautogui.hotkey("win", "2")
                                maximize_win()
                ...
                # Wait for a period of time and then maximize the window
                maximize_win()

            # If the given account number is 1, simply wait for a period of time
        ...
        def pc_search():
            ...
            pyautogui.write(search_input, interval=random.uniform(0.01, 0.02))
        ...
        for i in list_of_customised_account_numbers:
            ...
            print(Fore.CYAN + Style.NORMAL + str(Cooldown_acc) + Style.RESET_ALL)
            for accounts in Cooldown_acc_copy:
                time_now = time.time()
                acc_no = accounts[0]
                cl_time_st = accounts[1]
                time_passed = time_now-cl_time_st
                if time.time() - accounts[1] >= cooldown_time:
                    Cooldown_acc.remove(accounts)
                    print(f"Account details: {accounts}, removed from cooldown list.")
                    print(Fore.CYAN + Style.NORMAL + str(Cooldown_acc) + Style.RESET_ALL)
                    profile(acc_no)
                    end(acc_no, rep=0)
                    if acc_no == 1:
                    ...
                    print(f"{cooldown_time-float("{:.2f}".format(time_passed))} seconds more for points to cooldown in account number {acc_no}")
        ...
        while Cooldown_acc:
            ...
            for accounts in Cooldown_acc_copy:
                ...
                if time.time() - accounts[1] >= cooldown_time:
                    ...
                    print(Fore.RED + Style.BRIGHT + f"{cooldown_time-float("{:.2f}".format(time_passed))} seconds more for points to cooldown in account number {acc_no}" + Style.RESET_ALL)
                if not least:
                    least = cooldown_time-(time.time()-accounts[1])
                elif cooldown_time-(time.time()-accounts[1]) < least:
                    least = cooldown_time-(time.time()-accounts[1])
            if least:
                if least >= 0:
                    print(Fore.CYAN + Style.NORMAL + str(Cooldown_acc) + Style.RESET_ALL)
                    print(time.time())
                    print(Fore.MAGENTA + Style.BRIGHT + f"Pausing program for {least} seconds." + Style.RESET_ALL)
                    time.sleep(least)
                else:
                    print(Fore.CYAN + Style.NORMAL + str(Cooldown_acc) + Style.RESET_ALL)
                    print(time.time())
...
IS = "Choose ACCOUNT :" + f"\n@ - All 2nd level accounts {Second_level_accounts}" + f"\n# - All 1st level Accounts {First_level_accounts}" + Manage_account(
    "IS") + "\nEnter: - "
