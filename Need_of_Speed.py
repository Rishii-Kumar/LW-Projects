...
def my_program():
    try:
                ...
                while dimensions is None:
                    ...
                    if rem_decision is not None:
                        ...
                        while rem_dec_tick is None:
                            rem_dec_tick = Bunch_img_det("E:\\Det_img\\TXT Files\\Remember_tick.txt",
                                                         code_string="print('Remember Decision Tick NOT FOUND!')",
                                                         img_name="Remember my decision tick")
                        ...
                        while and_points_path is None:
                            ...
                            if wait_path % 5 == 0:
                                ...
                                if etab is None:
                                    ...
                                    set_later = det_img("E:\\Det_img\\Set_later1.png",
                                                        code_string="pass", img_name="Set Later Button") or det_img(
                                        "E:\\Det_img\\Set_later2.png", code_string="pass",
                                        img_name="Set Later Button") or det_img(
        ...
        #         if not least:
        #             least = accounts[1]
        #         elif accounts[1] > least:
        #             least = accounts[1]
        #     if least:
        #         while time.time() - least <= cooldown_time:
        #             print(Fore.YELLOW + Style.BRIGHT + str(format_time(cooldown_time - (time.time() - least))) + Style.RESET_ALL)
        #             time.sleep(4)
        ...
        for i in list_of_customised_account_numbers:
            tim = time.time()
            print(Fore.RED +Style.BRIGHT + str(time.time()) + " :- Started profile\n" + Style.RESET_ALL)

            time_rep += str(Fore.RED +Style.BRIGHT + str(time.time()) + " :- Started profile\n" + Style.RESET_ALL)

            profile(i)

            time_rep += str(Fore.BLUE + Style.BRIGHT + str(time.time()) + " :- Completed profile\n" + Style.RESET_ALL)

            time_rep += str(Fore.YELLOW + Style.BRIGHT + str(time.time() - tim) + " :- time taken to complete profile\n" + Style.RESET_ALL)
            print(Fore.YELLOW + Style.BRIGHT + str(time.time() - tim) + " :- time taken to complete profile\n" + Style.RESET_ALL)

            print(Fore.BLUE + Style.BRIGHT + str(time.time()) + " :- Completed profile\n" + Style.RESET_ALL)

            print(Fore.RED + Style.BRIGHT + str(time.time()) + " :- Started End\n" + Style.RESET_ALL)

            timm = time.time()

            time_rep += str(Fore.RED +Style.BRIGHT + str(time.time()) + " :- Started end\n" + Style.RESET_ALL)

            end(i)

            time_rep += str(Fore.RED +Style.BRIGHT + str(time.time()) + " :- Ended End\n" + Style.RESET_ALL)
            print(Fore.BLUE +Style.BRIGHT + str(time.time()) + " :- Ended End\n" + Style.RESET_ALL)
            time_rep += str(Fore.YELLOW +Style.BRIGHT + str(time.time() - timm) + " :- time taken to complete End\n" + Style.RESET_ALL)
            print(Fore.YELLOW +Style.BRIGHT + str(time.time() - timm) + " :- time taken to complete End\n" + Style.RESET_ALL)
            ...
            time_rep += str(Fore.RED +Style.BRIGHT + str(time.time()) + " :- Completed first round\n" + Style.RESET_ALL)
            time_rep += str(Fore.YELLOW +Style.BRIGHT + str(time.time() - timm) + " :- time taken to complete first round\n" + Style.RESET_ALL)
            print(Fore.RED +Style.BRIGHT + str(time.time()) + " :- Completed first round\n" + Style.RESET_ALL)
            print(Fore.YELLOW +Style.BRIGHT + str(time.time() - timm) + " :- time taken to complete first round\n" + Style.RESET_ALL)
            print(time_rep)
        ...
        while Cooldown_acc:
            ...
            while time.time() - first_cooldwon_time <= cooldown_time:
                print(format_time(cooldown_time - (time.time() - first_cooldwon_time)))
                time.sleep(4)
